...
export class FindAdminWalletDetailsController {
  ...
  constructor(
    findWalletDetailsUseCase: IfindWalletUseCase,
    findTransactionDetailsUseCase: IfindTransactionsUseCase,
    ...
    try {
      ...
      if (!wallet?._id) {
        res
          ...
          .json({ message: Messages.WALLET_FETCH_ERROR });
        return;
      }

      const { transactions, totalPages } =
        await this.findTransactionDetailsUseCase.findTransactions(
          wallet._id,
          page,
        );
      res
      ...
      logError("Error while finding admin wallet details", error);
      handleErrorResponse(req, res, error, Messages.WALLET_FETCH_ERROR);
    }
  }
  async handleFindWalletByPaymentStatus(
    req: Request,
    res: Response,
  ): Promise<void> {
    try {
      ...
      logError("Error in handleFindTransactionsByPaymentStatus", error);
      handleErrorResponse(req, res, error, Messages.TRANSACTION_FETCH_ERROR);
    }
  }
}
